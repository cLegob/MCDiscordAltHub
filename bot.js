...
function botMaker() {
    try {
        minecraftBot = mineflayer.createBot({
            host: config.minecraftServer.host, // Server IP from config.json
            port: config.minecraftServer.port || 25565, // Default port if not provided
            username: process.env.EMAIL, // Using environment variable from ecosystem.json
            password: process.env.PASSWORD, // Using environment variable from ecosystem.json
            auth: process.env.AUTH_TYPE || 'microsoft', // Authentication type from ecosystem.json
        });
...
// Discord bot login
discordBot.login(process.env.BOT_TOKEN); // Using environment variable from ecosystem.json

// ID variables from config.json
const discordServerID = config.discordServerID;
